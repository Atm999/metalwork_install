    ...
    try:
        ...
        if len(frame_list) != 0:
          for frame in frame_list:
            if frame['orgId'] == data['orgId'] and frame['srpName'] == data['srpName'] and frame['language'] == data['language']:
              print("srpName:%s, orgId:%s, language:%s, exist!"%(data['srpName'],data['orgId'],data['language']))
              return True
...
if __name__=='__main__':
    ...
    if config_datasource_replace != "":
      config_datasource_replace_list = str.split(config_datasource_replace, "***")
    ...
    if config_srpframe_replace != "":
      config_srpframe_replace_list = str.split(config_srpframe_replace, "***")
                ...
                if (options.type != None) and (options.type == 'datasource'):
                    ...
                    if srpframe_replace_count != 0:
                      for i in range(0, srpframe_replace_count):
                        srpframe_data = srpframe_data.replace(config_srpframe_replace_array[i][0], config_srpframe_replace_array[i][1])
